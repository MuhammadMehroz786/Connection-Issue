...
class Product(db.Model):
    ...
    def to_shopify_format(self):
        ...
        # If no options, ensure variants have option1='Default Title'
        if not options:
            # No real options - ensure all variants have option1='Default Title'
            for variant in unique_variants:
                if 'option1' not in variant or not variant['option1']:
                    variant['option1'] = 'Default Title'
            options = [{'name': 'Title'}]
            logger.info(f"✅ No variant options - using default Title option with 'Default Title' value")
        else:
...
class ProductVariant(db.Model):
    ...
    def to_shopify_format(self):
        ...
        # Multiply price by 2 for markup
        try:
            price_value = float(self.price) * 2
            price_str = f"{price_value:.2f}"
        except (ValueError, TypeError):
            price_str = self.price  # Fallback to original if conversion fails

        variant = {
            'price': price_str,  # Price multiplied by 2
            'requires_shipping': self.requires_shipping,
            'taxable': self.taxable,
            'inventory_management': None,
            'inventory_policy': 'continue',
        ...
        if self.compare_at_price:
            # Multiply compare_at_price by 2 as well
            try:
                compare_price_value = float(self.compare_at_price) * 2
                variant['compare_at_price'] = f"{compare_price_value:.2f}"
            except (ValueError, TypeError):
        ...
        if not options:
            ...
            for variant in unique_variants:
                if 'option1' not in variant or not variant['option1']:
                    variant['option1'] = 'Default Title'
            options = [{'name': 'Title'}]
            logger.info(f"✅ AI Product: No variant options - using default Title option with 'Default Title' value")
        else:
            logger.info(f"✅ Built {len(options)} option(s) for Shopify with names: {[o['name'] for o in options]}")
...
class AIJob(db.Model):
    ...
    custom_access_token = db.Column(db.String(255))
