...
class GeminiService:
    ...
    def _ensure_client_initialized(self, key_index):
        ...
        with self._init_lock:
            ...
            try:
                ...
                # Use v1beta API to access image generation models like gemini-2.5-flash-image
                client = genai.Client(api_key=key, http_options={'api_version': 'v1beta'})
                self.clients[key_index] = client
                logger.info(f"✅ Lazy-initialized Gemini Client #{key_index + 1} (using v1beta API for image models)")
    ...
    def edit_product_image(self, original_image_url, product_title, variation="main", all_image_urls=None):
        ...
        try:
            ...
            # Skip SVG and other non-raster image formats
            if original_image_url.lower().endswith(('.svg', '.webp', '.ico', '.gif')):
                logger.warning(f"⚠️ Skipping unsupported image format: {original_image_url}")
                return None

            # Download primary image (handle both URLs and data URLs)
            try:
            ...
            try:
                ...
                logger.error(f"❌ Cannot open image (possibly SVG or corrupted): {str(e)}")
                return None
            ...
            if width != height:
                ...
                right = left + size
                bottom = top + size
                primary_image = primary_image.crop((left, top, right, bottom))
                ...
                for idx, img_url in enumerate(all_image_urls):
                    try:
                        if img_url != original_image_url:  # Skip primary image (already loaded)
                            ...
                            # Crop context image to 1:1 as well
                            ctx_width, ctx_height = context_img.size
                            if ctx_width != ctx_height:
                                ctx_size = min(ctx_width, ctx_height)
                                ctx_left = (ctx_width - ctx_size) // 2
                                ctx_top = (ctx_height - ctx_size) // 2
                                context_img = context_img.crop((ctx_left, ctx_top, ctx_left + ctx_size, ctx_top + ctx_size))
            ...
            if response.candidates and len(response.candidates) > 0:
                ...
                if hasattr(candidate, 'finish_reason'):
                    ...
                    if 'SAFETY' in finish_reason or 'RECITATION' in finish_reason or 'BLOCKED' in finish_reason:
                        logger.warning(f"⚠️ Gemini [{key_name}]: Image rejected by safety filters - {finish_reason}")
                    ...
                    if hasattr(candidate.content, 'parts') and candidate.content.parts is not None:
                        for part in candidate.content.parts:
                            if hasattr(part, 'inline_data') and part.inline_data is not None:
                                ...
                                logger.info(f"✅ Gemini [{key_name}]: Successfully edited image (variation: {variation})")
                                return data_url
                    else:
                        logger.error(f"❌ Gemini returned text instead of image for: {product_title}")
                        logger.error(f"   Model 'gemini-2.5-flash' does NOT support image generation")
                        logger.error(f"   It can only ANALYZE images and return TEXT")
                        logger.error(f"   You need access to Imagen API or use DALL-E for image generation")
                        return None

            logger.error(f"❌ No edited image data found in Gemini response for: {product_title}")
            logger.error(f"   This usually means the model doesn't support image output")
            return None
