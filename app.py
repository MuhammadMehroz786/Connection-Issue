...
REQUIRED_ENV_VARS = {
    ...
    'FIRECRAWL_API_KEY': 'Firecrawl API key for crawling any e-commerce website',
    'OPENAI_API_KEY': 'OpenAI API key for product extraction and content enhancement',
...
if DATABASE_URL:
    # PostgreSQL detected (Railway or production)
    ...
    DATABASE_PATH = os.getenv('DATABASE_PATH', 'shopify_automation.db')
    
    # Check if Railway volume path exists
...
# Image generation service selection - SEEDREAM
IMAGE_MODEL = 'seedream'  # Use SeeDream (doubao-seedream-4-5-251128) for image generation
logger.info("🎨 Using SeeDream (doubao-seedream-4-5-251128) for image generation")
from services.seedream_service import SeeDreamService
seedream_service = SeeDreamService(os.getenv('SEEDREAM_API_KEY', '').strip())
image_service = seedream_service
...
product_grouper = ProductVariantGrouper(similarity_threshold=0.85)
...
def start_firecrawl_scrape():
    """Start Firecrawl scraping for any e-commerce website and save to database"""
    try:
        data = request.get_json()
        url = data.get('url')
        ...
        logger.info(f"🔥 Starting Firecrawl for URL: {url}, task_id: {task_id}")

        # Start Firecrawl workflow asynchronously with app context
        executor.submit(run_firecrawl_workflow_with_context, task_id, url, max_pages)
        ...
        logger.error(f"Error starting Firecrawl scrape: {str(e)}", exc_info=True)
...
@login_required
def create_ai_dupes():
    """Create AI-enhanced dupes from source products using OpenAI + Flux"""
    try:
        data = request.get_json()
        product_ids = data.get('product_ids', [])
        ...
        for product_id in product_ids:
            ...
            try:
                ...
                # Collect all image URLs for context (filter out SVG and unsupported formats)
                all_image_urls = [
                    img.original_url for img in all_images 
                    if img.original_url and not img.original_url.lower().endswith(('.svg', '.webp', '.ico', '.gif'))
                ]
                if not all_image_urls:
                    logger.error(f"❌ No valid image URLs for product {product_id} (all images are SVG/unsupported) - SKIPPING")
                    errors.append(f"Product {product_id}: No valid image URLs (only SVG/unsupported formats)")
                    continue

                # Use first VALID image as primary, pass all for context
                primary_image_url = all_image_urls[0]
                logger.info(f"📸 Found {len(all_image_urls)} valid image(s) for product - using all for context")

                # Image 1: Product in use (clean, no workers/hands/tools)
                logger.info(f"Editing Image 1 with {IMAGE_MODEL.upper()}: Product in use (clean, no workers)...")
                ...
                if not edited_url_1:
                    logger.error(f"❌ {IMAGE_MODEL.upper()} edit failed for image 1/2 on product {product_id} - SKIPPING (no fallback allowed)")
                    errors.append(f"Product {product_id}: {IMAGE_MODEL.upper()} image editing failed (image 1/2 - product in use)")
                    continue

                ai_image_urls.append(edited_url_1)
                logger.info(f"✅ {IMAGE_MODEL.upper()}: Edited image 1/2 (Product in use - clean)")
                ...
                logger.info(f"Editing Image 2 with {IMAGE_MODEL.upper()}: {second_image_variation} scene - using Image 1 as base...")
                ...
                if not edited_url_2:
                    logger.error(f"❌ {IMAGE_MODEL.upper()} edit failed for image 2/2 on product {product_id} - SKIPPING (no fallback allowed)")
                    errors.append(f"Product {product_id}: {IMAGE_MODEL.upper()} image editing failed (image 2/2 - {second_image_variation})")
                    continue
                ...
                logger.info(f"✅ {IMAGE_MODEL.upper()}: Edited image 2/2 ({variation_name})")

                image_prompt = f"{IMAGE_MODEL.upper()} edited variations of {source_product.title}"
                ...
                for variant in source_product.variants:
                    ...
                    # Handle 'null' strings and invalid values
                    variant_price = 0
                    if variant.price and variant.price not in ['null', 'None', '']:
                        try:
                    ...
                    # Handle compare_at_price - filter out 'null' strings and invalid values
                    variant_compare_price = None
                    if variant.compare_at_price and variant.compare_at_price not in ['null', 'None', '']:
                        try:
                            variant_compare_price = float(variant.compare_at_price)
                        except (ValueError, TypeError):
                            variant_compare_price = None
        ...
        response_message = f'Created {created_count} AI-enhanced product dupe(s) using OpenAI + {IMAGE_MODEL.upper()}'
        if errors:
...
def process_single_product(source_product, ai_job_id, fast_mode, created_counter, pushed_counter, source_url=None):
    ...
    with app.app_context():
        try:
            ...
            price = "0.00"
            try:
                variants_list = source_product.variants.all()
                if variants_list and len(variants_list) > 0:
                    first_variant = variants_list[0]
                    price = first_variant.price or "0.00"
            except Exception as e:
                logger.warning(f"[AI Job {ai_job_id}] Could not get variant price: {e}")
                price = "0.00"
            ...
            # CRITICAL: If image generation fails, STOP - do NOT create product
            if not edited_url_1:
                error_msg = f"{IMAGE_MODEL.upper()} image editing failed (image 1/2) - SKIPPING product (no fallback allowed)"
                logger.error(f"[AI Job {ai_job_id}] ❌ {error_msg}")
                return (False, error_msg)

            ai_image_urls.append(edited_url_1)
            logger.info(f"[AI Job {ai_job_id}] ✅ {IMAGE_MODEL.upper()}: Image 1/2 edited successfully (Product in use)")
            ...
            # CRITICAL: If image generation fails, STOP - do NOT create product
            if not edited_url_2:
                error_msg = f"{IMAGE_MODEL.upper()} image editing failed (image 2/2) - SKIPPING product (no fallback allowed)"
                logger.error(f"[AI Job {ai_job_id}] ❌ {error_msg}")
                return (False, error_msg)

            ai_image_urls.append(edited_url_2)
            logger.info(f"[AI Job {ai_job_id}] ✅ {IMAGE_MODEL.upper()}: Image 2/2 edited successfully (Installation scene)")
    ...
    with app.app_context():
        try:
            ...
            if quota_exhausted and failed_due_to_quota:
                ...
                # Quota handling (only for Gemini)
                if IMAGE_MODEL == 'gemini':
                    seconds_until_reset, reset_time = image_service._calculate_quota_reset_time()
                    ...
                    image_service.reset_quota_flags()
                else:
                    # Flux doesn't have quota limits, so just log and continue
                    logger.info(f"[AI Job {ai_job_id}] ⚠️ Quota exhaustion (Flux doesn't have quotas, this shouldn't happen)")
...
def run_firecrawl_workflow_with_context(task_id, url, max_pages):
    """Wrapper to run Firecrawl workflow with Flask app context"""
    with app.app_context():
        run_firecrawl_workflow(task_id, url, max_pages)


def run_firecrawl_workflow(task_id, url, max_pages):
    """
    Firecrawl workflow: Crawl any e-commerce website, extract products with OpenAI, and save to database
    STEP 1: Crawl website with Firecrawl
    STEP 2: Use OpenAI to identify product pages and extract structured data
    STEP 3: Save products to database
    """
    try:
        logger.info(f"[{task_id}] 🔥 Starting Firecrawl workflow for URL: {url}")

        # Update job status
        db_service.update_scrape_job(task_id, status='running')

        # STEP 1: Start Firecrawl crawl
        logger.info(f"[{task_id}] 🔥 STEP 1: Starting Firecrawl crawl for {url}")
        crawl_id = firecrawl_service.start_crawl(url, max_pages=max_pages)

        if not crawl_id:
            logger.error(f"[{task_id}] Failed to start Firecrawl")
            db_service.update_scrape_job(
                task_id,
                status='failed',
                error_message='Failed to start Firecrawl',
                completed_at=datetime.utcnow()
            )
            return
        ...
        if not success:
            ...
            db_service.update_scrape_job(
                task_id,
                status='failed',
                error_message='Firecrawl failed or timed out',
                completed_at=datetime.utcnow()
            )
            return

        logger.info(f"[{task_id}] ✅ Firecrawl completed successfully!")

        # Get crawled pages
        logger.info(f"[{task_id}] 📦 Fetching crawled pages from Firecrawl")
        pages = firecrawl_service.get_crawled_pages(crawl_id)

        if not pages:
            logger.warning(f"[{task_id}] No pages found in Firecrawl crawl {crawl_id}")
            db_service.update_scrape_job(
                task_id,
                status='completed',
                total_products=0,
        ...
        logger.info(f"[{task_id}] ✅ Fetched {len(pages)} pages from Firecrawl")

        # STEP 2: Extract products using OpenAI (batch processing for large datasets)
        logger.info(f"[{task_id}] 🤖 STEP 2: Extracting products from pages using OpenAI...")
        logger.info(f"[{task_id}] 📊 Processing {len(pages)} pages in batches...")
        
        extracted_products = []
        batch_size = 50  # Process 50 pages at a time to avoid memory issues
        
        for batch_start in range(0, len(pages), batch_size):
            batch_end = min(batch_start + batch_size, len(pages))
            batch_pages = pages[batch_start:batch_end]
            
            logger.info(f"[{task_id}] 📦 Processing batch {batch_start//batch_size + 1}/{(len(pages) + batch_size - 1)//batch_size} (pages {batch_start+1}-{batch_end})")
            
            batch_products = product_extractor.extract_products_from_pages_simple(batch_pages)
            extracted_products.extend(batch_products)
            
            logger.info(f"[{task_id}] ✅ Batch complete: {len(batch_products)} products extracted (total: {len(extracted_products)})")
        ...
        if not extracted_products:
            logger.warning(f"[{task_id}] No products extracted from pages")
            db_service.update_scrape_job(
                task_id,
                status='completed',
                total_products=0,
        ...
        logger.info(f"[{task_id}] ✅ Extracted {len(extracted_products)} products")
        
        # STEP 2.5: Group products with variants using Python grouper
        logger.info(f"[{task_id}] 🔗 STEP 2.5: Grouping products with variants...")
        grouped_products = product_grouper.group_products(extracted_products)
        
        logger.info(f"[{task_id}] ✅ Grouped into {len(grouped_products)} unique products")
        
        # Convert grouped products to flat list for database
        products = []
        for group in grouped_products:
            parent = group['parent_product']['product_data']
            
            # If there are variants, merge them into the parent
            if group['total_variants'] > 1:
                # Collect all variants
                all_variants = [parent.get('variants', [{}])[0]]  # Parent's variant
                
                for variant_info in group['variants']:
                    variant_product = variant_info['product_data']
                    variant_data = variant_product.get('variants', [{}])[0]
                    # Update variant title to be distinctive
                    variant_data['title'] = variant_product.get('title', 'Variant')
                    all_variants.append(variant_data)
                
                # Update parent with all variants
                parent['variants'] = all_variants
                
                # Use cleaner title (from group identifiers)
                if group['group_identifiers']['base_title']:
                    # Capitalize first letter of each word
                    clean_title = ' '.join(word.capitalize() for word in group['group_identifiers']['base_title'].split())
                    parent['title'] = clean_title
                
                logger.info(f"[{task_id}] 📦 Merged product: '{parent['title']}' with {len(all_variants)} variants")
        ...
        logger.info(f"[{task_id}] ✅ Extracted {len(products)} products from {len(pages)} pages")
        ...
        for idx, product_data in enumerate(products, 1):
            try:
                logger.info(f"[{task_id}] Processing product {idx}/{len(products)}: {product_data.get('title', 'Untitled')}")

                # Product data is already in Shopify format from OpenAI extraction
                # Just need to format it properly for database
                ...
                logger.info(f"[{task_id}] 📸 Product '{product_data.get('title')}' has {len(image_urls)} images")
                if image_urls:
                    logger.info(f"[{task_id}]    First image: {image_urls[0][:100]}")
                ...
                # Only use placeholder if NO images were extracted
                if not shopify_product['images']:
                    logger.warning(f"[{task_id}] ⚠️  No images found for '{product_data.get('title')}', using placeholder")
                    shopify_product['images'] = ['https://via.placeholder.com/500x500?text=No+Image']
...
def push_product_to_shopify(product_id):
    ...
    try:
        ...
        if product_images:
            logger.info(f"📸 Adding {len(product_images)} actual product images to Shopify...")
            for idx, image in enumerate(product_images, 1):
                image_url = image.original_url if hasattr(image, 'original_url') else None
                if image_url and image_url.startswith('http'):
                    success = shopify_service.add_product_image(shopify_product_id, image_url)
                    if success:
                        logger.info(f"✅ Added image {idx}/{len(product_images)}: {image_url[:80]}...")
                    else:
                        logger.warning(f"⚠️  Failed to add image {idx}: {image_url[:80]}...")
                    time.sleep(0.5)  # Small delay between image uploads
        else:
            logger.warning(f"⚠️  No images found for product {product_id}, using placeholder")
